...
function tokenize(question: string): string[] {
  ...
  for (const word of words(question)) {
    ...
    if (synonym) for (const part of synonym.split(' ')) out.add(stem(part));
...
function searchSections(question: string, sections: Section[]): Section[] {
  ...
  return sections
    .map(section => {
      ...
      if (LEGAL.has(section.page)) score -= 8;
...
function askEl<T extends HTMLElement>(id: string): T {
  const found = document.getElementById(id);
  if (!found) throw new Error(`ask: #${id} is missing`);
  return found as T;
}
...
function addBubble(role: 'you' | 'studio', text: string): HTMLDivElement {
...
function addLinks(links: AskLink[]): void {
...
function setMode(text: string | null): void {
...
function reasonFor(detail: string, status: number): string {
...
async function send(question: string): Promise<void> {
  ...
  try {
    ...
    if (response.ok) {
      const data = (await response.json()) as AskReply;
...
async function fallback(question: string, pending: HTMLDivElement): Promise<void> {
  pending.classList.remove('ask-pending');
...
function focusables(panel: HTMLElement): HTMLElement[] {
  return [...panel.querySelectorAll<HTMLElement>('button, input, a[href]')]
...
function initAsk(): void {
  ...
  panel.querySelector<HTMLButtonElement>('.ask-close')?.addEventListener('click', () => togglePanel(false));

  document.addEventListener('keydown', event => {
    if (event.key === 'Escape' && !panel.hidden) togglePanel(false);
  });

  for (const opener of document.querySelectorAll<HTMLElement>('[data-ask-open]')) {
  ...
  askEl<HTMLFormElement>('ask-form').addEventListener('submit', event => {
    event.preventDefault();
    const input = askEl<HTMLInputElement>('ask-input');
  ...
  for (const suggestion of document.querySelectorAll<HTMLButtonElement>('#ask-suggestions button')) {
